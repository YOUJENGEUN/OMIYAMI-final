document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateTotalOrderAmount() {
    ...
    // hidden input 값 업데이트 (checkout.jsp로 넘길 값)
	document.getElementById('hiddenTotalOrderAmount').value = totalAmount;
	document.getElementById('hiddenShippingFee').value = shippingFee;
	document.getElementById('hiddenFinalPaymentAmount').value = finalAmount;
	document.getElementById('hiddenRewardPoint').value = rewardPoints;
  }

  // 선택된 상품만 checkout 페이지로 전송하는 이벤트 추가
  checkoutForm.addEventListener('submit', function(event) {
    // 선택된 체크박스만 폼에 추가
    var selectedProductIds = Array.from(itemCheckboxes).filter(cb => cb.checked).map(cb => cb.value);
